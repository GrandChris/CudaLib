...
/// </summary>
template<typename T>
typename std::unique_ptr<T, cudaFree_deleter> dp_unique_ptr;
...
template<typename T>
std::unique_ptr<T, cudaFree_deleter> dp_make_unique(size_t const size);
...
template<typename T>
inline void cudaFree_deleter::operator()(T * p) noexcept
{
	...
	cudaError_t const error = cudaFree(p);
...
template<typename T>
inline std::unique_ptr<T, cudaFree_deleter> dp_make_unique(size_t const size)
{
	using remove_extent_t = typename std::remove_extent<T>::type;

	remove_extent_t * dp = nullptr;
	CUDA_CHECK( cudaMalloc(&dp, size * sizeof(remove_extent_t) ));
	return std::unique_ptr<T, cudaFree_deleter>(dp);
}
